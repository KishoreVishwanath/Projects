...
public class WrkOrdOptJFLineChartView extends DemoView {
	 ...
	 * @param context
	 */
	public WrkOrdOptJFLineChartView(Context context, Map<String, Integer> s,
			Map<String, Integer> b) {
		super(context);

		CategoryDataset dataset = createDataset(s, b);
	...
	private static CategoryDataset createDataset(Map<String, Integer> to,
			Map<String, Integer> ben) {

		// row keys...
		String series1 = "Job Flow";
		String series2 = "Benchmark Job Flow";
		...
		for (Map.Entry<String, Integer> entry : to.entrySet()) {
			if (i % 4 == 0) {
				...
				Integer fthing = ben.get(key.substring(0,2));
				dataset.addValue(fthing, series2, key);
			}
			i++;
		}
	...
	private static AFreeChart createChart(CategoryDataset dataset) {
		...
		AFreeChart chart = ChartFactory.createLineChart("TMI Transactions", // chart
				// title
				"Optimized Job Flow History for Today", // domain axis label
